...
class Board:
    ...
    def find_lowest_risk(self):
        for box_size in reversed(range(self.size)):
            coordinates = [cs for cs in self.risk_map if
                           (cs[0] == box_size and cs[1] >= box_size) or
                           (cs[0] >= box_size and cs[1] == box_size) or
                           (cs[0] == box_size + 1 and cs[1] >= box_size + 1) or
                           (cs[0] >= box_size + 1 and cs[1] == box_size + 1)]
            risk_map_box = {}
    ...
    def find_lowest_risk_in_box(self, curr_coordinate: Tuple[int, int], risk_map: Dict[Tuple[int, int], int]) -> int:
        min_risk = self.min_risk_cache.get(curr_coordinate, None)
        if min_risk is None:
